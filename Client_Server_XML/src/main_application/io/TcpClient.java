...
public class TcpClient
{
			...
			Document loginRequestXML) throws IOException
	{
		...
		try
		{
			...
			{
				logger.severe("Issue with xmlToObject method.  Exception: "
						+ e1);
			}
	...
	private Document generateLoginRequest()
	{
		...
		while(!isValidLoginResponseInput)
		{
			...
			{
				logger.severe("Issue with objectToXml method.  Exception: " + e);
			}
		}
		return dom;
	...
	private void startSession() throws IOException
	{
		...
		while(continueTesting)
		{
			...
			// Create a LoginRequest object from the users input and then turn
			// that into an
			// XML object.
